...
def letter_to_binary(array1):
    d = {"w":-1, None:0, "b": 1}
    array = [[0 for i in range(8)] for j in range(8)]
    
    for i in range(8):
        for j in range(8):
            array[i][j] = d[array1[i][j]]

    return array

def binary_to_letter(array1):
    d = {1:"w", 0:None, -1:"b"} 
    array = [[0 for i in range(8)] for j in range(8)]
...
##                print(alphaBetaResult[1])
##
##                if len(alphaBetaResult)==3:
##                    position = alphaBetaResult[2]
##                    self.oldarray[position[0]][position[1]]="b"

                move = controller.population[0].policy(letter_to_binary(flip(self.array)), controller.env)

                controller.env.step(move)

                self.array = flip(copy.deepcopy(binary_to_letter(controller.env.board)))
